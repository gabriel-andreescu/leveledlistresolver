...
namespace leveledlistresolver
{
        ...
        where TSet : class, IMajorRecord, TGet
    {
        bool Try(
            ...
            FormKey formKey,
            [MaybeNullWhen(false)] out TSet? setter
        );
    }
    ...
    sealed partial class RecordPatcher
    {
        ...
        public static bool TryPatch<TGet, TSet>(
            IPatcherState<ISkyrimMod, ISkyrimModGetter> state,
            TGet getter,
            [NotNullWhen(true)] out TSet? setter
        )
            where TGet : class, IMajorRecordGetter
            where TSet : class, IMajorRecord, TGet
        {
            var instance =
                Instance as IRecordPatcher<TGet, TSet> ?? throw new NotImplementedException();
